...
Базовый idle-loop, бухты, лодки, здания, рыбы, заказы, коллекция, фестиваль, оффлайн-доход, split mobile/yandex, стартовые 3D-like ассеты, ежедневная награда, экспедиции, мастерство бухт, наборы аквариума, жемчужные бонусы и цели главы.
...
1. Довести экспедиции:
   - нормальные награды по биомам;
   - редкие находки;
   - получение завершенной экспедиции после таймера;
   - позже: rewarded ad для ускорения.

2. Добавить milestone-награды:
   - лодки каждые 5/10 уровней;
   - здания каждые 5/10 уровней;
   - награды: звезды, жетоны, сундуки, постоянные малые множители.

3. Добавить событие/регату:
   - жетоны события уже есть в состоянии;
   - нужен магазин события;
   - недельные цели;
   - использовать `regatta-event-banner.svg`.

4. Сделать первый tutorial:
   - продать рыбу;
   - улучшить лодку;
   - открыть заказы;
   - открыть маяк;
